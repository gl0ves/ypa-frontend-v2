<script lang="ts">
	...
	import {updatePropertyAlert, deletePropertyAlert} from '$lib/api';
	...
	const update = async () => {
		const response = await updatePropertyAlert(formData, identifier);
		if(response.ok) {
			formSubmitted = true;
		} else {
...
<div class="flex flex-col items-center justify-center h-full py-5">
	...
	>
		...
		<Button
		variant="destructive"
		on:click={() => deleteAllPropertyAlerts}
		type="submit">DELETE ALERT</Button
	>
	
		<Button
		variant="destructive"
		on:click={() => deleteAllPropertyAlerts(true)}
		type="submit">DELETE ALL ALERTS</Button
	>
